...
####################
# Mosaicking line-of-sight velocity frames
#
...
if __name__ == "__main__":
    ...
    compiled_gps = load_gps()
    tarim_gps = load_tarim()
    all_gps = pd.concat([dummy_gps, compiled_gps, tarim_gps], ignore_index=True)
    plt.show()

    # 1. list all frames with the correct suffix in the directory
    tifList = sorted(glob.glob(os.path.join(input_dir, '*' + input_suffix)))
    print(tifList)
