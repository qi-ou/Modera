...
# - lines 658-716: Change input parameters to read your own data files.
#
...
# Wright et al., (2026). High-resolution geodetic velocities reveal role of weak faults in deformation of Tibetan Plateau. Science (New York, N.Y.), 391(6784), 499–503. https://doi.org/10.1126/SCIENCE.ADI3552
# Elliott et al., (2026). Deformation, strains and velocities for the Alpine Himalayan Belt from trans-continental Sentinel-1 InSAR & GNSS. Remote Sensing of Environment, 338(9), 115320. https://doi.org/10.1016/j.rse.2026.115320
...
def main():
    ...
    if predict_los:
        # turn one layer to nan
        if noA1:
            predict_suffix = "_noA1"
            a0los = copy.copy(asc_los_0)
            asc_los_0 = asc_los_0 * np.nan
        if noA2:
            predict_suffix = "_noA2"
            a1los = copy.copy(asc_los_1)
            asc_los_1 = asc_los_1 * np.nan
        if noD1:
            predict_suffix = "_noD1"
            d0los = copy.copy(dsc_los_0)
            dsc_los_0 = dsc_los_0 * np.nan
        if noD2:
            predict_suffix = "_noD2"
    ...
    if predict_los:
        ...
        if noA1:
            ...
            im = x1.imshow(asc_los_0_diff, vmin=vmin, vmax=vmax, cmap=cm.bwr, interpolation="nearest")
            divider = make_axes_locatable(x1)
            cax = divider.append_axes("right", size="3%", pad="2%")
            c = plt.colorbar(im, cax=cax)
            c.set_label('mm/yr')
            x1.set_title('A1 diff = {:.1f}±{:.1f} mm/yr'.format(np.nanmean(asc_los_0_diff), np.nanstd(asc_los_0_diff)))
        ...
        if noA2:
            ...
            im = x2.imshow(asc_los_1_diff, vmin=vmin, vmax=vmax, cmap=cm.bwr, interpolation="nearest")
            divider = make_axes_locatable(x2)
            cax = divider.append_axes("right", size="3%", pad="2%")
            c = plt.colorbar(im, cax=cax)            
            c.set_label('mm/yr')
            x2.set_title('A2 diff = {:.1f}±{:.1f} mm/yr'.format(np.nanmean(asc_los_1_diff), np.nanstd(asc_los_1_diff)))
        ...
        if noD1:
            ...
            im = x3.imshow(dsc_los_0_diff, vmin=vmin, vmax=vmax, cmap=cm.bwr, interpolation="nearest")
            divider = make_axes_locatable(x3)
            cax = divider.append_axes("right", size="3%", pad="2%")
            c = plt.colorbar(im, cax=cax)            
            c.set_label('mm/yr')
            x3.set_title('D1 diff = {:.1f}±{:.1f} mm/yr'.format(np.nanmean(dsc_los_0_diff), np.nanstd(dsc_los_0_diff)))
        ...
        if noD2:
            ...
            im = x4.imshow(dsc_los_1_diff, vmin=vmin, vmax=vmax, cmap=cm.bwr, interpolation="nearest")
            divider = make_axes_locatable(x4)
            cax = divider.append_axes("right", size="3%", pad="2%")
            c = plt.colorbar(im, cax=cax)
            c.set_label('mm/yr')
            x4.set_title('D2 diff = {:.1f}±{:.1f} mm/yr'.format(np.nanmean(dsc_los_1_diff), np.nanstd(dsc_los_1_diff)))
            export_tif(dsc_los_1_diff, maps, output_dir + 'd2_diff{}.tif'.format(output_suffix))


 
    if export_ve_vu:
        export_tif(ve, maps, output_dir+'ve{}{}.tif'.format(output_suffix,predict_suffix))
        export_tif(vu, maps, output_dir+'vu{}{}.tif'.format(output_suffix,predict_suffix))
        export_tif(ve_sig, maps, output_dir+'ve_sig{}{}.tif'.format(output_suffix,predict_suffix))
        export_tif(vu_sig, maps, output_dir+'vu_sig{}{}.tif'.format(output_suffix,predict_suffix))
        ...
        fig2, [[ax1, ax2], [ax3, ax4]] = plt.subplots(2, 2, sharex='all', sharey='all', figsize=(6.8, 4))
        im = ax1.imshow(ve, vmin=ve_vmin, vmax=ve_vmax, extent=(maps.left, maps.right, maps.bottom, maps.top), interpolation="nearest")
        divider = make_axes_locatable(ax1)
        cax = divider.append_axes("right", size="3%", pad="2%")
        c = plt.colorbar(im, cax=cax)
        c.set_label('mm/yr')
        ax1.set_title('Ve')
        im = ax2.imshow(vu, vmin=vu_vmin, vmax=vu_vmax, extent=(maps.left, maps.right, maps.bottom, maps.top), interpolation="nearest")
        divider = make_axes_locatable(ax2)
        cax = divider.append_axes("right", size="3%", pad="2%")
        c = plt.colorbar(im, cax=cax)
        c.set_label('mm/yr')
        ax2.set_title('Vu')
        im = ax3.imshow(ve_sig, vmin=ve_sig_vmin, vmax=ve_sig_vmax, extent=(maps.left, maps.right, maps.bottom, maps.top), interpolation="nearest")
        divider = make_axes_locatable(ax3)
        cax = divider.append_axes("right", size="3%", pad="2%")
        c = plt.colorbar(im, cax=cax)
        c.set_label('mm/yr')
        ax3.set_title(r'$\sigma$(Ve) = {:.1f}±{:.1f} mm/yr'.format(np.nanmean(ve_sig), np.nanstd(ve_sig)))
        im = ax4.imshow(vu_sig, vmin=vu_sig_vmin, vmax=vu_sig_vmax, extent=(maps.left, maps.right, maps.bottom, maps.top), interpolation="nearest")
        divider = make_axes_locatable(ax4)
        cax = divider.append_axes("right", size="3%", pad="2%")
        c = plt.colorbar(im, cax=cax)
        c.set_label('mm/yr')
        ax4.set_title(r'$\sigma$(Vu) = {:.1f}±{:.1f} mm/yr'.format(np.nanmean(vu_sig), np.nanstd(vu_sig)))
        fig2.savefig(output_dir+'ve_vu{}{}.png'.format(output_suffix,predict_suffix), format='PNG', dpi=300, bbox_inches='tight')


def half_geometry_stats():
        
    ################################################################
    # histograms of ve diff and vu diff from half geometry
    ###############################################################

    ve_A1_D1=output_dir+"ve_{}_A1_D1.tif".format(area)
    vu_A1_D1=output_dir+"vu_{}_A1_D1.tif".format(area)
    ve_A2_D2=output_dir+"ve_{}_A2_D2.tif".format(area)
    vu_A2_D2=output_dir+"vu_{}_A2_D2.tif".format(area)
    ve_A1_D2=output_dir+"ve_{}_A1_D2.tif".format(area)
    vu_A1_D2=output_dir+"vu_{}_A1_D2.tif".format(area)
    ve_A2_D1=output_dir+"ve_{}_A2_D1.tif".format(area)
    vu_A2_D1=output_dir+"vu_{}_A2_D1.tif".format(area)

    ve_a1d1=OpenTif(ve_A1_D1)
    vu_a1d1=OpenTif(vu_A1_D1)
    ve_a2d2=OpenTif(ve_A2_D2)
    vu_a2d2=OpenTif(vu_A2_D2)
    ve_a1d2=OpenTif(ve_A1_D2)
    vu_a1d2=OpenTif(vu_A1_D2)
    ve_a2d1=OpenTif(ve_A2_D1)
    vu_a2d1=OpenTif(vu_A2_D1)

    ve_diff_a1d1_a2d2 = ve_a1d1.data - ve_a2d2.data
    vu_diff_a1d1_a2d2 = vu_a1d1.data - vu_a2d2.data
    ve_diff_a1d2_a2d1 = ve_a1d2.data - ve_a2d1.data
    vu_diff_a1d2_a2d1 = vu_a1d2.data - vu_a2d1.data

    export_tif(ve_diff_a1d1_a2d2, ve_a1d1, output_dir+"ve_diff_a1d1_a2d2.tif")
    export_tif(vu_diff_a1d1_a2d2, ve_a1d1, output_dir+"vu_diff_a1d1_a2d2.tif")
    export_tif(ve_diff_a1d2_a2d1, ve_a1d1, output_dir+"ve_diff_a1d2_a2d1.tif")
    export_tif(vu_diff_a1d2_a2d1, ve_a1d1, output_dir+"vu_diff_a1d2_a2d1.tif")

    plt.rcParams['font.size'] = 12

    fig, ax = plt.subplots(1,4, sharey='all', figsize=(12, 3))

    ax[0].hist(ve_diff_a1d1_a2d2.flatten(), bins=np.arange(-10, 10.1, 0.1), label="RMS={:.1f}".format(np.nanstd(ve_diff_a1d1_a2d2)), alpha=.5)
    ax[0].legend()
    ax[0].set_title("Ve_diff_A1D1-A2D2")
    ax[0].set_xlabel("Ve diff, mm/yr")
    ax[0].set_ylabel("Pixel Count")

    ax[1].hist(vu_diff_a1d1_a2d2.flatten(), bins=np.arange(-10, 10.1, 0.1), label="RMS={:.1f}".format(np.nanstd(vu_diff_a1d1_a2d2)), alpha=.5)
    ax[1].legend()
    ax[1].set_title("Vu_diff_A1D1-A2D2")
    ax[1].set_xlabel("Vu diff, mm/yr")

    ax[2].hist(ve_diff_a1d2_a2d1.flatten(), bins=np.arange(-10, 10.1, 0.1), label="RMS={:.1f}".format(np.nanstd(ve_diff_a1d2_a2d1)), alpha=.5)
    ax[2].legend()
    ax[2].set_title("Ve_diff_A1D2-A2D1")
    ax[2].set_xlabel("Ve diff, mm/yr")

    ax[3].hist(vu_diff_a1d2_a2d1.flatten(), bins=np.arange(-10, 10.1, 0.1), label="RMS={:.1f}".format(np.nanstd(vu_diff_a1d2_a2d1)), alpha=.5)
    ax[3].legend()
    ax[3].set_title("Vu_diff_A1D2-A2D1")
    ax[3].set_xlabel("Vu diff, mm/yr")

    # plt.show()
    plt.tight_layout()
    fig.savefig(output_dir+"ve_vu_diff_stats.png", format='PNG', dpi=300, bbox_inches='tight', pad_inches=0.01)
...
if __name__ == "__main__":
    ...
    area="TianShan"
    output_dir = "../los_full/uncertainty/"
    ...
    if predict_los:
        ...
        fig, [[x1, x2], [x3, x4]] = plt.subplots(2, 2, sharex='all', sharey='all')
        plt.suptitle("LOS_diff"+output_suffix)

        # List to hold variable references (in this case, names, since Python booleans are immutable)
        vars_names = ['noA1', 'noA2', 'noD1', 'noD2']

        # Loop through each variable, setting only one to True at a time
        for var in vars_names:
            # Reset all variables to False
            noA1, noA2, noD1, noD2 = False, False, False, False
            
            # Set the current variable to True using locals() to access variable by name
            locals()[var] = True
            
            # Print the state to see the effect
            print(f"Status - noA1: {noA1}, noA2: {noA2}, noD1: {noD1}, noD2: {noD2}")
            
            main()

        # plt.show()
        fig.savefig(output_dir+"LOS_diff{}.png".format(output_suffix), format='PNG', dpi=300, bbox_inches='tight')

    elif half_geometry:

        noA1, noA2, noD1, noD2, geometry_suffix = True, False, True, False, '_A2_D2'
        print(f"Status - noA1: {noA1}, noA2: {noA2}, noD1: {noD1}, noD2: {noD2}")
        output_suffix = "_" + area + geometry_suffix
        main()

        noA1, noA2, noD1, noD2, geometry_suffix = False, True, True, False, '_A1_D2'
        print(f"Status - noA1: {noA1}, noA2: {noA2}, noD1: {noD1}, noD2: {noD2}")
        output_suffix = "_" + area + geometry_suffix
        main()

        noA1, noA2, noD1, noD2, geometry_suffix = True, False, False, True, '_A2_D1'
        print(f"Status - noA1: {noA1}, noA2: {noA2}, noD1: {noD1}, noD2: {noD2}")
        output_suffix = "_" + area + geometry_suffix
        main()

        noA1, noA2, noD1, noD2, geometry_suffix = False, True, False, True, '_A1_D1'
        print(f"Status - noA1: {noA1}, noA2: {noA2}, noD1: {noD1}, noD2: {noD2}")
        output_suffix = "_" + area + geometry_suffix
        main()
